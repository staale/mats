      ...
      group('authorization invalid when Server about to receive or send out information bearing message', () {
        Future testIt(String userId, bool reauthBeforeRecieve) async {
          ...
          if (reauthBeforeRecieve) {
            ...
            authCallbackCalledEventType = await authEventCompleter.future;

            // We expect the reauth to occur before receive, thus receive should not be complete yet
            ...
            // 1. Wait for the recieve event to occur first
            await receivedEventCompleter.future;

            // At this point, the auth callback should not have been called yet
          ...
          // -- At this point, both receive and auth callback has been invoked, so we can now assert
          //    the type of authentication, as well as that the reply should not be done yet.
